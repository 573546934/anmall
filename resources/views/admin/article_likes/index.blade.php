        ...
        <script>
            layui.use(['layer','table','form'],function () {
                ...
                var dataTable = table.render({
                    ...
                    ,url: "{{ route('admin.article_likes.data') }}" //数据接口
                    ,page: true //开启分页
                    ,totalRow: true
                    ,cols: [[ //表头
                        ...
                        ,{field: 'type', title: '访问类型', toolbar:'#type',width:100}
                        ,{field: 'fid', title: '分享人ID ',width:100}
                        ,{field: 'f', title: '分享用户昵称',toolbar:'#f',width:120}
                        ,{field: 'num', title: '访问次数',width:120,sort: true, totalRow: true}
                        ,{field: 'mid', title: '访问用户ID',width:100}
                ...
                table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'del'){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.article_likes.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                                if (result.code==0){
                ...
                $("#listDelete").click(function () {
                    ...
                    if (ids.length>0){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.article_likes.destroy') }}",{_method:'delete',ids:ids},function (result) {
                                if (result.code==0){
                                    dataTable.reload()
                                }
                                layer.close(index);
