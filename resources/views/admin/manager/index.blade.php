        ...
        <script>
            layui.use(['layer','table','form'],function () {
                ...
                var dataTable = table.render({
                    ...
                    ,url: "{{ route('admin.manager.data') }}" //数据接口
                    ,page: true //开启分页
                    ,cols: [[ //表头
                ...
                table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'del'){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.manager.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                                if (result.code==0){
                        ...
                        layer.confirm('确认审核？', {
                            ...
                            $.post("{{ route('admin.manager.by') }}",{_method:'put',ids:[data.id]},function (result) {
                                if(result.code == 0){
                                    dataTable.reload()
                            ...
                            $.post("{{ route('admin.manager.refuse') }}",{_method:'put',ids:[data.id]},function (result) {
                                if(result.code == 0){
                                    dataTable.reload()
                                    layer.msg(result.msg,{icon:6})
                                }else{
                ...
                $("#listDelete").click(function () {
                    ...
                    if (ids.length>0){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.manager.destroy') }}",{_method:'delete',ids:ids},function (result) {
                                if (result.code==0){
                                    dataTable.reload()
                                }
                                layer.close(index);
                ...
                $("#listBy").click(function () {
                    ...
                    if (ids.length>0){
                        layer.confirm('确认审核？', {
                            ...
                            $.post("{{ route('admin.manager.by') }}",{_method:'put',ids:ids},function (result) {
                                if(result.code == 0){
                                    dataTable.reload()
                                    layer.msg(result.msg,{icon:6})
                                }else{
                            ...
                            $.post("{{ route('admin.manager.refuse') }}",{_method:'put',ids:ids},function (result) {
                                if(result.code == 0){
                                    dataTable.reload()
                                    layer.msg(result.msg,{icon:6})
                                }else{
